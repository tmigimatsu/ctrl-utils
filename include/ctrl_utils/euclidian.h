 ...
 *
 * @see Python: ctrlutils.orientation_error()
 */
 ...
 *
 * Unlike `ctrl_utils::OrientationError()`, this error ignores rotations about
 ...
 * @return Quaternion error.
 *
 * @see Python: ctrlutils.lookat_error()
 */
 ...
 * farther quarternion will cause it to rotate the long way around, which may be
 * desired to avoid singularities, for example.
 *
 * @param quat Quaternion representing the desired orientation.
 * @param quat_ref Reference quaternion.
 *
 * @returns The quaternion that represents the same orientation as `quat` but
 *          has a positive dot product with `quat_ref`.
 *
 * @see Python: ctrlutils.near_quaternion()
 */
 ...
 * reference quaternion.
 *
 * @param ori 3D matrix representing the desired orientation.
 * @param quat_ref Reference quaternion.
 *
 * @returns The quaternion that represents the same orientation as `ori` but
 *          has a positive dot product with `quat_ref`.
 *
 * @see Python: ctrlutils.near_quaternion()
 */
 ...
 *
 * @returns The quaternion that represents the same orientation as `quat` but
 *          has a negative dot product with `quat_ref`.
 *
 * @see Python: ctrlutils.far_quaternion()
 */
...
// Eigen::AngleAxisd& aa) {
  ...
  /**
   * Compute the Jacobian of a rotated matrix R * X with respect to the
   * rotation's exponential coordinates (angle axis representation).
   *
   * If X is a vector, the Jacobian returned is d(Rx)/dw. If X is a matrix, the
   * Jacobian tensor is stacked into a matrix [d(RX)/dw_x; d(RX)/dw_y;
   * d(RX)/dw_z].
   *
